...
internal sealed class ArmorItemGameDefinitionReader : GameDefinitionReader<ItemDefinition>
{
    public override string FileName { get; } = "armor.txt";
    ...
    protected override ItemDefinition? Read(IReader reader)
    {
        ...
        var itemDefinition = ItemDefinition.Create(code!, ItemType.Armor, name, gradeType, version, level, isStackable, size);
        ...
        if (gradeType != ItemGradeType.Exceptional)
        {
            itemDefinition.AddGrade(exceptionalCode, ItemGradeType.Exceptional);
        }

        if (gradeType != ItemGradeType.Elite)
        {
